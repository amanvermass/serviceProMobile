...
export default function RenewHostingScreen() {
  const { id, name, renewalDate } = useLocalSearchParams();
  ...
  const [formData, setFormData] = useState({
    renewalDate: renewalDate ? new Date(renewalDate as string).toISOString().split('T')[0] : '',
    monthlyCost: '',
    remark: '',
  });

  const handleSave = async () => {
    if (!formData.renewalDate || !formData.monthlyCost) {
    ...
    try {
      ...
      await hostingApi.renewHosting(id as string, payload);
      setToastMessage(`Hosting renewed successfully!`);
      setTimeout(() => {
        router.back();
      }, 1200);
    } catch (error) {
      console.error('Error renewing hosting:', error);
      setToastMessage('Failed to renew hosting. Please check your connection.');
      setTimeout(() => setToastMessage(''), 2000);
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.headerBlock}>
          <Server size={40} color="#059669" style={styles.headerIcon} />
          <Text style={styles.domainName}>{name}</Text>
          <Text style={styles.subtitle}>Current Renewal: {renewalDate ? new Date(renewalDate as string).toLocaleDateString() : 'N/A'}</Text>
        </View>
